...
def cal_information_gain(data, a):
    ...
    temp = Ent - gain
    gain_ration = temp / IV_a
    return (temp, gain_ration)
...
#挑选最优属性，在信息增益大于平均水平的属性中选取增益率最高的
def get_best_feature(data):
    #取出当前所有属性
    features = data.columns[:-1]
    res = {}
    for a in features:
        res[a] = cal_information_gain(data, a) #信息增益和增益率
    res = sorted(res.items(), key=lambda x: x[1][0], reverse=True) #按信息增益排名，items表示把字典转换成列表， x[1]表示按value排序，而且保留key和value
    res_avg = sum([x[1][0] for x in res])/len(res) #信息增益平均值
    good_res = [x for x in res if x[1][0] >= res_avg] #高于平均水平的
    res = sorted(good_res, key=lambda x: x[1][1], reverse=True) #按增益率排序
    return res[0][0]
